...
MPMSolver2D::MPMSolver2D(const Vector2& origin, Float cellSize, Int nI, Int nJ, SceneObjects* sceneObjs) :
    _objects{sceneObjs},
    _particles{cellSize},
...
void MPMSolver2D::generateParticles(const SDFObject& sdfObj, Float initialVelocity_y) {
...
void MPMSolver2D::addRepulsiveVelocity(const Vector2& p0, const Vector2& p1, Float dt, Float radius, Float magnitude) {
...
void MPMSolver2D::advanceFrame(Float frameDuration) {
    Float frameTime = 0;
...
Float MPMSolver2D::timestepCFL() const {
    Float maxVel = 0;
...
void MPMSolver2D::moveParticles(Float dt) {
...
void MPMSolver2D::collectParticlesToCells() {
...
void MPMSolver2D::particleVelocity2Grid() {
...
void MPMSolver2D::extrapolate(Array2X<Float>& grid, Array2X<Float>& tmp_grid, Array2X<char>& valid, Array2X<char>& old_valid) const {
    tmp_grid  = grid;
    old_valid = valid;
...
void MPMSolver2D::addGravity(Float dt) {
    _grid.v.loop2D([&](std::size_t i, std::size_t j) {
                       if(_grid.vValid(i, j)) {
...
void MPMSolver2D::computeFluidSDF() {
...
void MPMSolver2D::solvePressures(Float dt) {
...
void MPMSolver2D::constrainVelocity() {
    _grid.uTmp = _grid.u;
    _grid.vTmp = _grid.v;
...
void MPMSolver2D::relaxParticlePositions(Float dt) {
...
void MPMSolver2D::gridVelocity2Particle() {
